#!/usr/bin/env bash
# GOP 60, P-frames only (no B-frames): bf=0, medium CRF 30, 45% scale.
set -euo pipefail
...
head -n "$(wc -l < "$VIDEO_NAMES_FILE")" "$VIDEO_NAMES_FILE" | xargs -P"$JOBS" -I{} bash -c '
  ...
  ffmpeg -nostdin -y -hide_banner -loglevel warning \
    ...
    -g 60 -bf 0 -x265-params "keyint=60:min-keyint=1:scenecut=40:frame-threads=4:log-level=warning" \
    -r 20 "$OUT"
' _ {}
