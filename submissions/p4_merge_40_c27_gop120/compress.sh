#!/usr/bin/env bash
# Phase 4 GOP sweep B: same as P3-M1 but GOP 120 (1 I-frame / 6s @ 20fps).
set -euo pipefail
...
head -n "$(wc -l < "$VIDEO_NAMES_FILE")" "$VIDEO_NAMES_FILE" | xargs -P"$JOBS" -I{} bash -c '
  ...
  ffmpeg -nostdin -y -hide_banner -loglevel warning \
    ...
    -g 120 -bf 2 -x265-params "keyint=120:min-keyint=1:scenecut=40:frame-threads=4:log-level=warning" \
    -r 20 "$OUT"
' _ {}
