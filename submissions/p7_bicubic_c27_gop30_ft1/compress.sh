#!/usr/bin/env bash
# Phase 7: champion p6_bicubic_c27_ft1 + GOP 30 (keyint=30).
set -euo pipefail
...
head -n "$(wc -l < "$VIDEO_NAMES_FILE")" "$VIDEO_NAMES_FILE" | xargs -P"$JOBS" -I{} bash -c '
  ...
  ffmpeg -nostdin -y -hide_banner -loglevel warning \
    ...
    -g 30 -bf 0 -x265-params "keyint=30:min-keyint=1:scenecut=40:no-sao=1:frame-threads=1:log-level=warning" \
    -r 20 "$OUT"
' _ {}
